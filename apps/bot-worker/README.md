...
## Phase 1 checklist
1. Keep the checked-in `wrangler.toml` buildable. Add deployment-specific Cloudflare account settings through Wrangler login, CI secrets, or an uncommitted environment config.
...
- Create a Workers KV namespace (suggested name `dashboard-metrics`) with `wrangler kv:namespace create dashboard-metrics --preview`, then add a real `DASHBOARD_KV` binding in your deployment environment. Do not commit empty placeholder namespace IDs.
